<template>
  <div class="home mt-5">
    <!-- Category Container -->
    <v-container>
      <v-row v-if="$store.state.categories == null" dense>
          ...
          >
            <div class="align-self-center icon">
              <svg
                ...
                style="enable-background: new 0 0 54.819 54.819"
                xml:space="preserve"
              >
                <g>
                  <path
      ...
      <v-row v-if="$store.state.products != null" dense>
        <v-col v-for="(product, i) in $store.state.products" :key="i" cols="6" sm="3">
          <v-card flat elevation="1">
            ...
            <v-btn block elevation="0" color="primary" class="mt-1" @click="$store.commit('addToCart')">Add to cart</v-btn>
          </v-card>
        </v-col>
      </v-row>
      <v-row v-else>
